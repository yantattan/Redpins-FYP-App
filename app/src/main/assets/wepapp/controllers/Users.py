...
class UserCon:
    ...
    def Login(self, user):
        ...
        if userDBInfo is not None:
            ...
            if userVal is not None:
                return {"userId": userVal["_id"], "username": userVal["Username"], "role": userVal["Role"], "setupComplete": userVal["SetupComplete"]}
    ...
    def Register(self, user):
        ...
        passwordHash = hashlib.sha512((user.getPassword() + passwordSalt).encode("utf-8")).hexdigest()
        try:
            checkExist = self.__connection.find_one({"$or": [{"Username": user.getUsername()}, {"Email": user.getEmail()}, {"Contact": user.getContact()}] })
    ...
    def SetSetupComplete(self, userId):
        try:
            self.__connection.update_one({"_id": userId}, {"$set": {"SetupComplete": True}})
            userVal = self.__connection.find_one({"_id": userId})
            if userVal is not None:
                return {"userId": userVal["_id"], "username": userVal["Username"], "role": userVal["Role"], "setupComplete": userVal["SetupComplete"]}
        except Exception as e:
            print(e)
